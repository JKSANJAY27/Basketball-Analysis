...
class BallAcquisitionDetector:
    ...
    def get_key_basketball_player_assignment_points(self, player_bbox, ball_center):
        ...
        output_points += [(x1, y1), (x2, y1), (x1, y2), (x2, y2), (x1+width//2, y1), (x1+width//2, y2), (x1, y1+height//2), (x2, y1+height//2)]
    ...
    def find_best_candidate_for_acquisition(self, ball_center, player_tracks_frame, ball_bbox):
        ...
        for player_id, player_info in player_tracks_frame.items():
            ...
            distance_to_ball = self.find_minimum_distance_to_ball(ball_center, player_bbox)
            containment_ratio = self.calculate_ball_containment_ratio(player_bbox, ball_bbox)
            if containment_ratio > self.containment_threshold:
                high_containment_players.append((player_id, distance_to_ball))
            else:
                regular_distance_players.append((player_id, distance_to_ball))
    ...
    def detect_ball_possession(self, player_tracks, ball_tracks):
        ...
        for frame_num in range(num_frames):
            ...
            best_player_id = self.find_best_candidate_for_acquisition(ball_center, player_tracks[frame_num], ball_bbox)
            if best_player_id != -1:
